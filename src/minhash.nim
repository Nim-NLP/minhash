...
type
    TokenizerProc = proc (content: string): seq[string] {.gcsafe.}
    MinHasher*[T] = object
        tokenizer: TokenizerProc
        seeds:seq[uint32]
        num_seeds:int
...
proc slide*(content: string): seq[string] {.gcsafe.} =
    let maxLen = max(content.len - slideWidth + 1, 1)
    result = newSeq[string](maxLen)
    for i in 0..<maxLen:
        result[i] = content[i..<(i + slideWidth)]

proc minhash64*(str: string, seeds: openArray[uint32], tokenizer: TokenizerProc): seq[uint64] {.gcsafe, noInit.} =
    let num_seeds = seeds.len
    var
        hashes:array[2,uint64]
        curHash:uint64
    result = newSeq[UINT64_MAX](num_seeds)
    let ngrams = tokenizer(str)
    for s in 0..<num_seeds:
        curHash = UINT64_MAX
        for n in ngrams:
            MurmurHash3_x64_128(n, n.len, seeds[s], hashes)
            if hashes[0] < curHash:
                curHash = hashes[0]
        result[s] = curHash

proc minhash32*(str: string, seeds: openArray[uint32], tokenizer: TokenizerProc): seq[uint32] {.gcsafe, noInit.} =
    let num_seeds = seeds.len
    var
    ...
    for s in 0..<num_seeds:
        curHash = UINT32_MAX
        for n in ngrams:
            MurmurHash3_x86_32(n, n.len, seeds[s], hashes)
            if hashes[0] < curHash:
                curHash = hashes[0]
        result[s] = curHash
...
proc jaccard*[T](self: MinHasher[T], doc1, doc2: string): float =
    let
        f_a = toHashSet(self.fingerprint(doc1))
        f_b = toHashSet(self.fingerprint(doc2))
...
proc jaccard*[T](self: MinHasher[T], fingerprint1, fingerprint2: openArray[T]): float =
    let
        f_a = toHashSet(fingerprint1)
        f_b = toHashSet(fingerprint2)
    return len(f_a.intersection(f_b)) / len(f_a.union(f_b))

proc initMinHasher*[T](seeds: seq[SomeInteger], tokenizer = slide): MinHasher[T] =
    result.tokenizer = tokenizer
    result.seeds = cast[seq[uint32]](seeds)
...
proc add*[T](self: LocalitySensitive[T], fingerprint: seq[T], doc_id: string) =
    ...
    for bin_i, bucket in self.getBins(fingerprint).pairs:
        if self.bins[bin_i].len > 0 and self.bins[bin_i].hasKey(bucket):
            ...
            discard self.bins[bin_i].mgetOrPut(bucket, toHashSet([doc_id]))
...
proc remove*[T](self: LocalitySensitive[T], doc_id: string) =
    ...
    for bin_i, bucket in self.getBins(fingerprint).pairs:
        self.bins[bin_i][bucket].excl doc_id
...
proc getDuplicates*[T](self: LocalitySensitive[T], min_jaccard = 0.0): HashSet[tuple[a:string,b:string]] {.noInit.} =
    var candidates = initHashSet[tuple[a:string,b:string]]()
